...
#     owners =  ["390683858216"]  # account number of the owner of the ami. On the ec2 console click AMI under 
#                                 # image section and search your ami id in public images.
#     filter {
#       name = "name"
#       values = ["amzn2-ami-kernel-5.10-hvm-2.0.20230727.0-x86_64-gp2"] # filter by ami name
#     }
# }
...
resource "aws_instance" "linux_server" {
  ...
  ami = "ami-09538990a0c4fe9be"     # data.aws_ami.linux.id 

  # REFERENCE THE VALUE OF THE VARIABLE SPECIFIED
  instance_type = "t2.micro"              # var.instance_type ["dev"]  
  # Data.aws_availability_zone.example.name_suffix) # REFERENCE THE VALUE OF THE VARIABLE SPECIFIED
  availability_zone = data.aws_availability_zone.example.name 
  # REFERENCE THE VALUE OF THE VARIABLE SPECIFIED
  key_name = "playground"              # var.key_name 
